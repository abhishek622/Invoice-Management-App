...
public class Analytics extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		queryBuilder.append(
				"SELECT business_name, SUM(total_open_amount) AS total_open_amount, invoice_currency, COUNT(invoice_currency) AS no_of_customers FROM business, winter_internship AS wi");
		if (isNotEmpty(start_clear_date)) {
			whereCause.add("(wi.clear_date BETWEEN '" + start_clear_date + "' AND '" + end_clear_date + "')");
		}

		if (isNotEmpty(start_due_date)) {
			whereCause.add("(wi.due_in_date BETWEEN '" + start_due_date + "' AND '" + end_due_date + "')");
		}
		if (isNotEmpty(start_baseline_create_date)) {
			whereCause.add("(wi.baseline_create_date BETWEEN '" + start_baseline_create_date + "' AND '"
