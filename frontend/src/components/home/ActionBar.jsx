...
import { AddCustomer, Analytics, DelCustomer, EditCustomer, AdvSearch } from "../dialogs";
...
function ActionBar({ fetchData, prevRef }) {
	...
	const handlePredict = async () => {
		...
		if (get_prediction?.length) {
			for (let i = 0; i < get_prediction.length; i++) {
				...
				let ab =
					get_prediction[i].aging_bucket === "nan" ? "Before Due" : get_prediction[i].aging_bucket;
		...
		if (not_predicted_row.length > 0) {
			for (let row of not_predicted_row) {
				...
				const ab = prediction[0].aging_bucket === "nan" ? "Before Due" : prediction[0].aging_bucket;
				data_to_add.aging_buckets.push(ab);
		...
		fetchData({
			filed: tblHandler.sortModel?.length ? tblHandler.sortModel[0].field : null,
	...
	return (
		<div>
			...
			<StyledGrid container spacing={2}>
				<Grid item xs={12} sm={12} md={5}>
					...
					>
						...
						<Button startIcon={<AssessmentOutlinedIcon />} onClick={handleAnalytics}>
							Analytics
						</Button>
						<Button
			...
			<EditCustomer open={state.editCustomer} onClose={handleEditCustomer} prevRef={prevRef} />
			<DelCustomer open={state.delCustomer} onClose={handleDelCustomer} prevRef={prevRef} />
		</div>
	);
}
